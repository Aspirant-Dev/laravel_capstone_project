...
<head>
  ...
  <body>
    <header class="clearfix">
      <h1>Your Order is ready to delivery</h1>

      <div id="notices">
        <div><h2>Good day {{ $name }}!</h2></div>
        <br>
        @if($orders->payment_method == 'COD')
            <div><h2>We just want to inform that your Order #{{ $tracking_no }} on {{ date('j F Y (g:i a)', strtotime($orders->created_at)) }} is going to delivery today.<br/><br/>Please prepare &#8369;{{ number_format($orders->total_price,2) }}  for your payment. </h2></div>
        @else
            <div><h2>We just want to inform that your Order #{{ $tracking_no }} on {{ date('j F Y (g:i a)', strtotime($orders->created_at)) }} is going to delivery today.</h2></div>
        @endif
      </div>
    </header>
    <main>
